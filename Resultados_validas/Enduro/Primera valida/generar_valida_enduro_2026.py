"""
Script para generar la página HTML de resultados de la I Válida Enduro 2026.
- Scratch: todas las columnas, al inicio de la página (delimitador ;).
- Resto: quitar Clase; Comentario se oculta y se muestra icono ℹ al lado de Pos. si tiene valor.
"""
...
def parse_csv(filepath, delimiter=None):
...
def parse_filename(filename):
    """Retorna (categoria, tipo, sort_key). Scratch retorna ('Scratch', 'Scratch', -1) para ponerlo primero."""
    name = filename.replace('.csv', '').strip()
    name = re.sub(r'\s*-\s*resultados\s*$', '', name, flags=re.I).strip()
    if re.match(r'^scratch\s*$', name, re.I):
        return ('Scratch', 'Scratch', -1)
    parts = [p.strip() for p in re.split(r'\s+-\s+', name, flags=re.I) if p.strip()]
    if len(parts) < 2:
        return (format_categoria_name(parts[0] if parts else name), "Carrera", 0)
    tipo_str = parts[-1].lower().strip()
    if 'final' in tipo_str:
        tipo, sort_key = 'Final', 0
    elif 'carrera 1' in tipo_str:
        tipo, sort_key = 'Carrera 1', 2
    elif 'carrera 2' in tipo_str:
        tipo, sort_key = 'Carrera 2', 3
    elif 'carrera' in tipo_str:
        tipo, sort_key = 'Carrera', 1
    else:
...
def format_categoria_name(name):
    ...
    for p in parts:
        if p:
            if p.upper() in ('A', 'B', '1', '2', '3'):
                result.append(p.upper() if p in '12' else p[0].upper() + p[1:].lower())
            else:
                result.append(p[0].upper() + p[1:].lower() if len(p) > 1 else p.upper())
    return ' '.join(result)

def get_category_sort_key(categoria):
    if categoria.lower() == 'scratch':
        return (-1, categoria)
    order = {
        'infantil enduro 1': 0, 'infantil enduro 2': 1, 'infantil enduro 3': 2,
        'inicio': 3, 'juvenil': 4, 'junior': 5, 'junior novatos': 6,
        'no racer': 7, 'femenino': 8,
        'enduro 1': 9, 'enduro 2': 10, 'enduro 3': 11,
        'master a': 12, 'master b': 13
    }
...
def render_row(row, comentario, search_attrs, with_comentario_icon=True):
...
def render_scratch_row(row, headers_len, search_attrs):
    """Scratch: todas las celdas, sin icono comentario."""
    pos_val = str(row[0]).strip() if row else ''
    pos_class = ' class="pos-1"' if pos_val == '1' else (' class="pos-2"' if pos_val == '2' else (' class="pos-3"' if pos_val == '3' else ''))
    html_parts = [f'<tr{pos_class}{search_attrs}>']
    for cell in row:
        html_parts.append(f'<td>{escape_html(cell)}</td>')
    html_parts.append('</tr>')
...
def generate_html():
    ...
    for filename in os.listdir(FILES_DIR):
        ...
        if not os.path.isfile(filepath):
            continue
        delimiter = ';' if 'scratch' in filename.lower() else None
        headers, rows = parse_csv(filepath, delimiter=delimiter)
    ...
    if scratch_data:
        categorias_para_html.append(('Scratch', 'scratch', scratch_data))
    for categoria in sorted_categorias:
        ...
        first_tipo = tablas[0][0] if tablas else "Carrera"
...
<head>
    ...
    <title>I Válida Enduro 2026 | FEDEMOTO</title>
...
<body>
    ...
    <div id="contenido-exportar" class="container">
        <header>
            <h1>I Válida Enduro 2026</h1>
            <p>Resultados por categoría</p>
        </header>
        <div class="toolbar">
...
'''
    ...
    for categoria, section_id, data in categorias_para_html:
        if categoria == 'Scratch':
            headers, rows = data
            html_content += f'''
            <div class="categoria-section" id="scratch" data-categoria-id="scratch">
            ...
            for row in rows:
                ...
                nom = row[2] if len(row) > 2 else ''
                search_attrs = f' data-numero="{escape_html(num)}" data-nombre="{escape_html(nom)}"'
                html_content += render_scratch_row(row, len(headers), search_attrs)
        ...
        for item in tablas:
            ...
            if tipo == 'Final':
                ...
                if final_data is None and not c1_data and not c2_data:
                    final_data = (headers, rows, comentarios)
        ...
        if final_data and (c1_data or c2_data):
            c_headers, c_rows = (c1_data or c2_data)[0], (c1_data or c2_data)[1]
            best_carreras = get_mejor_tm_carreras(c1_data, c2_data)
            if best_carreras:
        ...
        main_tipo = 'Final' if final_data and any(t[0] == 'Final' for t in tablas) else (tablas[0][0] if tablas else 'Carrera')
        ...
        if main_table:
            ...
            for i, row in enumerate(rows):
                ...
                html_content += render_row(row, com, search_attrs, with_comentario_icon=True)
            html_content += '</tbody></table></div></div>'
